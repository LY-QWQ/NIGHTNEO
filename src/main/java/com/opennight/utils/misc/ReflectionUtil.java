...
public final class ReflectionUtil {
    ...
    public static void triggerClientSwing(net.minecraft.world.entity.LivingEntity entity) {
        try {
            Field swingingField = findField(entity.getClass(), "swinging");
            Field swingTimeField = findField(entity.getClass(), "swingTime");
            swingingField.setBoolean(entity, true);
            swingTimeField.setInt(entity, 0);
        } catch (Exception ex) {
            ClientBase.logger.error("Failed to set client swing", ex);
        }
    }

    public static float getYRot(ServerboundMovePlayerPacket packet) {
        if (ClientBase.mc.gameMode == null) return 0.0f;
        Field field = findField(packet.getClass(), NightNeo.isMCPMapped ? "f_134121_" : "yRot");
        try {
    ...
    public static float getXRot(ServerboundMovePlayerPacket packet) {
        if (ClientBase.mc.gameMode == null) return 0.0f;
        Field field = findField(packet.getClass(), NightNeo.isMCPMapped ? "f_134122_" : "xRot");
        try {
            return field.getFloat(packet);
        } catch (Exception ex) {
    ...
    public static void setDepthBufferId(RenderTarget renderTarget, int depthBufferId) {
        Field field = findField(renderTarget.getClass(), NightNeo.isMCPMapped ? "f_83924_" : "depthBufferId");
        try {
    ...
    public static Container getBrewingStand(BrewingStandMenu menu) {
        ...
        Field field = findField(menu.getClass(), NightNeo.isMCPMapped ? "f_39086_" : "brewingStand");
        try {
