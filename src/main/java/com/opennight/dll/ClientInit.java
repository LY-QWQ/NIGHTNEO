...
import com.opennight.asm.Bootstrap;
 ...
 * jar onto the Forge GameClassLoader, so {@link NightNeo} and all 21 patch
 * handler classes share Minecraft's class loader.
 *
 * <p>This entry point intentionally does <b>not</b> construct {@link NightNeo}.
 ...
 * NightNeo} on the next tick. Letting the existing tick-driven lazy-init run
 * keeps the DLL path and the mod path identical from {@code NightNeo.init}
 * onwards.</p>
 */
public final class ClientInit {
    ...
    public static synchronized void start(String extractedJarPath) {
        ...
        try {
            ...
            LOGGER.info("NightNeo loader     = {}", NightNeo.class.getClassLoader());
            ...
            // performs the real NightNeo construction via its existing
            // lazy-init path.
            NightNeo.registerPatches();
            ClassAgent.installPatchesAndRetransform();

            LOGGER.info("ClientInit done. NightNeo will be constructed on the next Minecraft.tick.");
        } catch (Throwable t) {
