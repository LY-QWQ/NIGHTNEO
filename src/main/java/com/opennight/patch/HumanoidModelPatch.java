...
public class HumanoidModelPatch {
    public static CameraPitchEvent onPitchRender(LivingEntity entity, float pitch) {
        if (NightNeo.isReady() && entity == ClientBase.mc.player && ClientBase.mc.level != null) {
            return (CameraPitchEvent) NightNeo.instance.getEventBus().call(new CameraPitchEvent(pitch));
        }
