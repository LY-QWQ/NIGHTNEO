...
import com.opennight.event.impl.PreTickEvent;
...
import com.opennight.event.impl.StuckInBlockEvent;
...
public class EntityPatch {
    ...
    public static void onPush(Entity entity, CallbackInfo callbackInfo) {
        if (!NightNeo.isReady() || entity != ClientBase.mc.player || entity.isInWater()) return;
    ...
    public static void overwriteMoveRelative(Entity entity, float speed, Vec3 movement) throws Exception {
        ...
        if (NightNeo.isReady() && entity == ClientBase.mc.player) {
            NightNeo.instance.getEventBus().call(new PreTickEvent());
        }
    }
